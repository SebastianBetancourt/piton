...
;                                                                (map (lambda (l)
;                                                                 (cases linea l
;                                                                   (una-linea (i is orden) (una-linea '() (cons i is) orden)))) lineas))))))))
...
(define-datatype procedimiento procedimiento?
  (cerradura (nombre symbol?) (parametros (list-of symbol?)) (cuerpo (list-of expresion?))))
...
(define destructurar
           (lambda (expr) (cases expresion expr
                            ...
                            (lista (exprs) exprs)
                                        (else (eopl:error 'destructurar "expresion no destructurizable")))))
  ...
  (lambda (izq dor der amb)
    (let ((precedencia (list
      ...
      (let loop ((expr (aplanar-operacion-binaria izq dor der))
                 (dores precedencia))
        (if (null? dores) (evaluar-expresion (car expr) amb) (loop (procesar-un-operador expr (car dores) amb) (cdr dores)))))))
  ...
  (lambda (cion dor amb)
    (if (or (null? cion) (null? (cdr cion))) cion
        (if (equal? (cadr cion) dor)
            (procesar-un-operador (cons (procesar-operacion-binaria-primitiva (car cion) dor (caddr cion) amb) (cdddr cion)) dor amb)
            (cons (car cion) (cons (cadr cion) (procesar-un-operador (cddr cion) dor amb)))))))
...
(define procesar-operacion-binaria-primitiva
  (lambda (izq-a-evaluar dor der-a-evaluar amb)
    (let*
        ((evaluar (lambda (x) (destructurar (basex->basey (reestructurar (evaluar-expresion x amb)) 10))))
         (base (lambda (x)
                 (cases expresion (reestructurar (evaluar-expresion x amb))
                   (numero-literal (n) 10)
...
(define evaluar-operacion-unaria
  (lambda (dor preando amb)
    (evaluar-expresion
    (let ((ando (reestructurar (evaluar-expresion preando amb))))
    (cases operador-unario dor
      (sucesor () (procesar-operacion-binaria-primitiva ando (suma) (numero-literal 1) amb))
      (predecesor () (procesar-operacion-binaria-primitiva ando (resta) (numero-literal 1) amb))
      (longitud () (reestructurar (if (string? (destructurar preando)) (string-length (destructurar preando)) (length (destructurar preando)))))
      (negacion () (reestructurar (not (destructurar ando))))
      (range () (lista  (rango (destructurar ando)))))) amb)))
    ...
    (letrec ((evaluar-subexpresion
           (lambda (la-expresion)
             (cases expresion la-expresion
               ...
               (operacion-unaria (dor ando) (begin (evaluar-subexpresion ando el-ambiente) None))
               (un-return (expr) (evaluar-expresion expr el-ambiente))
               (definicion-procedimiento (id ids phints rhint cuerpo) (asignar-variable! id (cerradura id ids cuerpo) el-ambiente))
               ...
               (condicional (cond1 cons1 conds conss else) (evaluar-condicional
                                                   ...
                                                   (subprograma->list else) el-ambiente
                                                   evaluar-proc))
               (ciclo-for (id expr body) (evaluar-for id (evaluar-expresion expr el-ambiente) body el-ambiente evaluar-proc))))))
      (begin
...
(define evaluar-bloque
  (lambda (cuerpo new-ids new-vals amb)
    (begin
...
(define evaluar-condicional
  (lambda (conds conss else amb eval)
    (if
     (null? conds) (eval else amb)
    (let ((resultado (evaluar-expresion (car conds) amb)))
      (cond
        [(equal? resultado 'True) (eval (car conss) amb)]
        [(equal? resultado 'False) (evaluar-condicional (cdr conds) (cdr conss) else amb eval)]
...
(define evaluar-bloque-y-mostrar
  (lambda (cuerpo amb)
    (if (null? (cdr cuerpo)) (evaluar-expresion (car cuerpo) amb)
        (begin (display (evaluar-expresion (car cuerpo) amb))
                                           (display"\n")
                                           (evaluar-bloque-y-mostrar (cdr cuerpo) amb)))))

(define evaluar-for
  (lambda (i lst-str body amb eval)
    ...
    (let loop ((resto l))
        (if (null? (cdr resto))
            (begin
              (asignar-variable! i (evaluar-expresion (car resto) amb) amb)
              (eval (subprograma->list body) amb))
          (begin
            (asignar-variable! i (evaluar-expresion (car resto) amb) amb)
            (let ((resultado (eval (subprograma->list body) amb))) (if (isNone? resultado) 0 (begin (display resultado) (display "\n"))))
            (loop (cdr resto))))))))
...
(define evaluar-expresion
  (lambda (la-expresion el-ambiente)
     (cases expresion la-expresion
      ...
      (operacion-binaria (izq dor der) (evaluar-operacion-binaria izq dor der el-ambiente))
      (operacion-unaria (dor ando) (evaluar-operacion-unaria dor ando el-ambiente))
      (un-return (expr) (eopl:error 'SyntaxError "'return' outside function"))
      (definicion-procedimiento (id ids phints rhint cuerpo) (asignar-variable! id (cerradura id ids (subprograma->list cuerpo)) el-ambiente))
      ...
      (ciclo-for (id expr body) (evaluar-for id (evaluar-expresion expr el-ambiente) body el-ambiente evaluar-bloque-y-mostrar)))))
...
(define read-eval-loop
  (sllgen:make-rep-loop ">>> "
     (lambda (ugly) (begin (tipar-programa (procesar-prog ugly))(evaluar-programa (procesar-prog ugly))))
...
(define piton3
  (lambda () (begin
    (display "Pitón 3.2.0 (default, ")
    (display "Sep 27 2018, 17:25:39) \n[DrRacket 6.7 3m] on eopl\nType \"help\", \"copyright\", \"credits\" or \"license\" for more information.\n")
    (read-eval-loop))))
